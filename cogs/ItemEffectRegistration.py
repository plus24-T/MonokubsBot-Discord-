...
class ItemEffectRegistration(commands.Cog):#Cog名、任意だが分かりやすさのためにコマンドが一つなら頭大文字でクラス作成
    ...
    async def item_effect_registration(self, itx:discord.Interaction,効果:Literal["襲撃無効","二票持ち"],target_nickname:discord.Member):#ここが処理内容、必要な引数とか設定する
        if 効果=="襲撃無効":
            gv.get_chara_data(target_nickname).escorted=True
            await itx.response.send_message(f"{target_nickname.nick}に襲撃無効が付与されました")

        else:
            gv.get_chara_data(target_nickname).votes=2
            await itx.response.send_message(f"{target_nickname.nick}は次の投票で2票投票できます")
