...
class Hagakure_Select(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if gv.get_chara_data(self.values[0]).role == gv.CharaRole.MONOMI:
            await interaction.response.send_message(f"『{self.values[0]}』は モノミ だべ！")
        else:
            await interaction.response.send_message(f"『{self.values[0]}』は モノミ じゃねえべ！")
        await discord.utils.get(interaction.guild.channels,name="食堂").send("占いが終わりました")
        self.disabled=True
...
class Hagakure(commands.Cog):#コマンド名、頭大文字でクラス作成
    ...
    async def hagakure(self,interaction:discord.Interaction): 
        ...
        await interaction.response.send_message("葉隠康比呂が占っています、少々お待ちください")
        await discord.utils.get(interaction.guild.channels,name="葉隠康比呂").send(
            "判別の対象を選択してください",
