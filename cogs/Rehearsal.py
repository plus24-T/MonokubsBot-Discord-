...
class Rehearsal_Select(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            ...
            f"{discord.utils.get(interaction.guild.channels,name="食堂").mention}へお戻りください"
            )
        gv.table_data.day_count+=1
        await discord.utils.get(interaction.guild.channels,name=self.values[0]).send(
...
class Rehearsal(commands.Cog):#コマンド名、頭大文字でクラス作成
    ...
    async def rehearsal(self, interaction:discord.Interaction):#ここが処理内容、必要な引数とか設定する
